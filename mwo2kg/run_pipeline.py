...
def run_ner(data_folder, options, filenames):
    ...
    ner_processed_folder = os.path.join(
        data_folder, filenames["ner_processed_folder"]
    )
    ner_intermediate_folder = os.path.join(
        data_folder, filenames["ner_intermediate_folder"]
    )

    if check_boolean(options["retrain_ner_model"]):
        entity_recognition.build_data(
            train_filename=os.path.join(
                ner_input_folder, filenames["ner_data_train"]
            ),
            dev_filename=os.path.join(
                ner_input_folder, filenames["ner_data_dev"]
            ),
            test_filename=os.path.join(
                ner_input_folder, filenames["ner_data_test"]
            ),
            output_folder=ner_processed_folder,
        )
        entity_recognition.train_flair(data_folder=ner_processed_folder)

    entity_recognition.predict_entities(
        input_filename=os.path.join(data_folder, filenames["mwos_short_text"]),
        output_filename=os.path.join(
            ner_intermediate_folder, filenames["autolabelled_mwos_conll"]
        ),
    )
    entity_recognition.results_to_json(
        ...
        output_filename=os.path.join(
            ner_intermediate_folder, filenames["autolabelled_mwos"]
        ),
    )
...
def run_postprocessing(data_folder, options, filenames):
    ...
    postprocessing.postprocess(
        ...
        output_filename=os.path.join(
            ner_intermediate_folder,
            filenames["autolabelled_mwos_postprocessed"],
        ),
        output_filename_hierarchy=os.path.join(
            ner_intermediate_folder, filenames["entity_hierarchy"]
        ),
        run_failure_mode_grouping=check_boolean(
            options["run_failure_mode_grouping"]
        ),
    )
...
def run_triple_generation(data_folder, options, filenames):
    ...
    triple_generation.build_triples(
        ner_input_filename=os.path.join(
            ner_intermediate_folder,
            filenames["autolabelled_mwos_postprocessed"],
        ),
        mwo_input_filename=os.path.join(data_folder, filenames["mwos"]),
        entity_hierarchy_input_filename=os.path.join(
            ner_intermediate_folder, filenames["entity_hierarchy"]
        ),
        delay_accounting_data_filename=os.path.join(
            data_folder, filenames["ampla_data"]
        )
        if "ampla_data" in filenames
        else None,
    )
